...
LL discrete_logarithm(LL a, LL b, LL n){
	...
	LL tmp_remainder = a % n; // tmp_remainder = a^(exp) (mod n)
	...
	printf( "\n\nremainders1:\n" );
	for(auto it: remainders1){
		printf( "(%lld, %lld)\n", it.remainder, it.exponent );
	}
	
	printf( "\n\nremainders2:\n" );
	for(auto it: remainders2){
		printf( "(%lld, %lld)\n", it.remainder, it.exponent );
	}
	...
	printf( "\n\nmap_remainders1:\n" );
	for(auto it: map_remainders1){
		printf( "(%lld, %lld)\n", it.remainder, it.exponent );
	}
	
	printf( "\n\nmap_remainders2:\n" );
	for(auto it: map_remainders2){
		printf( "(%lld, %lld)\n", it.remainder, it.exponent );
	}
	...
	if( it1 != map_remainders1.end() ){
		printf( "\nWe found a remainder %lld in 1st map\nThe candidate for a result is %lld\n", b, it1 -> exponent );
		...
		printf( "\nWe didn't find a remainder %lld in 1st map\n\n", b );
	}
	...
	if( found_solution_in_1st_map == false ){ // if no solution found in 1st map
		...
		if( it2 != map_remainders2.end() ){
			printf( "\nWe found a remainder %lld in 2nd map\nThe candidate for a result is %lld\n", b, it2 -> exponent );
			...
			printf( "\nWe didn't find a remainder %lld in 2nd map\n\n", b );
		}	
	}
	else{
		printf( "\nWe don't search the 2nd map because we found solution in the 1st map.\n\n" );
	}
		...
		for(HASH_MAP::iterator it1 = map_remainders1.begin(); it1 != map_remainders1.end();  ++it1){
			...
			if( matching_remainder != -1 ){ // if a solution exists
				...
				if( it2 != map_remainders2.end() ){ // if found a matching remainder successively
					printf( "we found a remainder %lld in 2nd map\nThe candidate for a result is %lld\n", matching_remainder, (it1->exponent) + (it2->exponent) );
					...
					printf( "a matching remainder doesn't exist in 2nd map\n" );
				}	
			}
			else{ // no matching remainder
				printf( "a matching remainder doesn't exist\n" );
			}
		}
	}
	else{
		printf( "\nWe don't try to match remainders from the 1st and 2nd map because we found solution in the 1st map.\n\n" );	
	}
